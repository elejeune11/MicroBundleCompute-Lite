...
def corners_to_mask(img: np.ndarray, r0: int, r1: int, c0: int, c1: int):
...
def shrink_pair(v0: int, v1: int, sf: float):
...
def remove_pillar_region(mask: np.ndarray, clip_fraction: float = 0.5) -> np.ndarray:
...
def create_sub_domains(
    ...
    *,
    ...
    mask_removed_pillars = remove_pillar_region(mask, pillar_clip_fraction)
    # compute overall box
...
def is_in_box(box: np.ndarray, rr: int, cc: int):
...
def load_sub_domain_strain(folder_path: Path, fname=""):
...
def F_to_Ecc(F_rr: float, F_rc: float, F_cr: float, F_cc: float) -> float:
    """Given F, will compute E_cc for visualization."""
...
def pngs_sub_domain_strain(
    ...
    for beat in range(0, num_beats):
        ...
        for kk in range(start_idx, end_idx):
            ...
            cbar.set_label("Ecc strain (image column axis)", rotation=270)
            plt.axis("off")
...
def run_sub_domain_strain_analysis(
    ...
    *,
    ...
    # rotate mask
    rot_mask = ia.rotate_imgs_all([mask], ang, center_row, center_col)[0]
    # rotate tracking results
    rot_tracker_row_all, rot_tracker_col_all = ia.rotate_pts_all(tracker_row_all, tracker_col_all, rot_mat, center_row, center_col)
    # create sub-domains
    sub_domain_box_list, tile_dim_pix, num_tile_row, num_tile_col = create_sub_domains(rot_mask, pillar_clip_fraction=pillar_clip_fraction, shrink_row=shrink_row, shrink_col=shrink_col, tile_dim_pix=tile_dim_pix, num_tile_row=num_tile_row, num_tile_col=num_tile_col, tile_style=tile_style)
    # compute strain in each sub-domain
    sub_domain_F_all, sub_domain_row_all, sub_domain_col_all = compute_sub_domain_position_strain_all(rot_tracker_row_all, rot_tracker_col_all, sub_domain_box_list)
